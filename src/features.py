...
def engineer_features(data_df):
    ...
    age_segment = age_groups.astype(str) + "_" + df['segment'].astype(str)
    ...
    new_cols['age_segment'] = age_segment
    ...
    new_cols['bal_drawdown_severity'] = (new_cols['bal_max'] - new_cols['bal_min']) / (new_cols['bal_mean'] + 1.0)
    ...
    # Cumulative Area Under the Balance Curve
    ...
    # Balance Multi-Month Lags and Differences
    for i in range(1, 6):
    ...
    # Proximity to 6-month historical minimum balance
    ...
    for m in range(1, 7):
        ...
        # P2P Net Reliance Index per month
    ...
    # Cashflow Elasticity (Row-wise correlation between monthly inflows and outflows)
    inflow_mat_norm = inflow_matrix - np.mean(inflow_matrix, axis=1, keepdims=True)
    outflow_mat_norm = outflow_matrix - np.mean(outflow_matrix, axis=1, keepdims=True)
    inflow_std = np.std(inflow_matrix, axis=1) + 1e-5
    ...
    # Liquidity Exhaustion Days Countdown
    ...
    # Consecutive month-over-month Degradation Streaks
    degrade_streaks = sum(
    ...
    # -------------------------------------------------------------
    # 5. Transaction-Level Slopes, Ratios & Sudden Silence Flags
    # -------------------------------------------------------------
...
def fit_fold_unsupervised_personas(train_df, n_clusters=8, seed=SEED):
    """
    Fits KMeans behavioral personas strictly on training fold slice.
    """
    cluster_features = [
    ...
    kmeans.fit(sub_norm)
    return kmeans, (sub_mean, sub_std), avail_cols

def transform_fold_unsupervised_personas(df, kmeans, scaler_params, avail_cols):
    """
    Applies pre-fitted KMeans cluster distance transformations without data leakage.
    """
    sub_mean, sub_std = scaler_params
    sub_mat = np.nan_to_num(df[avail_cols].values, nan=0.0)
    sub_norm = (sub_mat - sub_mean) / sub_std
    cluster_labels = kmeans.predict(sub_norm)
    cluster_distances = kmeans.transform(sub_norm)
    
    out_df = df.copy()
    out_df['financial_persona_cluster'] = cluster_labels.astype(str)
    for c_i in range(kmeans.n_clusters):
        out_df[f'dist_to_persona_cluster_{c_i}'] = cluster_distances[:, c_i].astype(np.float32)
    return out_df
